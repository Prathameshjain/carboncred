...
const VerificationReport = ({ project, showDownloadButton = true }) => {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            if (project.blockchain_tx_hash) {
                const txHash = project.blockchain_tx_hash.startsWith('0x')
                    ? project.blockchain_tx_hash
                    : '0x' + project.blockchain_tx_hash;
                const etherscanUrl = `https://sepolia.etherscan.io/tx/${txHash}`;

                // Add clickable link annotation (approximately where "View on Etherscan" button is)
                const linkWidth = 50;
                const linkHeight = 10;
                const linkX = pdfWidth - linkWidth - 10;
                const linkY = pdfHeight * 0.34;
